...
class MarvinPhone(commands.Cog):
    ...
    @commands.command(
        name="text",
        help="Send a text to someone! You must get owner approval to use this command.",
    )
    async def text(self, ctx, recipient: str, *msg):
        ...
        if recipient.isalpha():
            ...
            search_results = await loop.run_in_executor(
                ThreadPoolExecutor(),
                self.bot.get_cog("AddressBook").retrieve_contacts_number,
                user,
                recipient,
            )
            if search_results["contact_found"]:
                if len(search_results["potential_contacts"]) > 1:
                    ...
                    contact_index = await self.bot.wait_for(
            ...
            if not validate_phone_number(recipient):
                await ctx.send(
                    "Your phone number was not in a valid format! Please try again. Goodbye."
                )
                return
        ...
        if msg == "":
            await ctx.send(
                "You must provide a message when you call this command! Please try again, goodbye."
            )
            return
        ...
        if message_count > self.MAX_MESSAGES:
            await ctx.send(
                "Your message is too long for me to send. I am limited to 5 messages of 160 characters each"
            )
        ...
        sent_message = await ctx.send(
            f"I have attempted to send your message.\nStatus: {message_content.status}"
                ...
                for id, results in text.items():
                    if results["status"] not in self.FINAL_DELIVERY_STATUSES:
                        ...
                        if response.status in self.FINAL_DELIVERY_STATUSES:
                            ...
                            await message.edit(
                                content=f"**UPDATE FOR YOUR MESSAGE TO: {response.to}.**\n"
