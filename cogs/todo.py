...
class ToDo(MarvinDB, commands.Cog):
    ...
    INSERT_TO_DO_FOR_USER = f"""INSERT INTO {TODO_TABLE_NAME} (user, item) VALUES(?,?);"""
    GET_TO_DO_LISTS = f"""SELECT * FROM {TODO_TABLE_NAME}"""
    UPDATE_TO_DO_FOR_USER = f"""UPDATE {TODO_TABLE_NAME} SET item=? WHERE user=?"""
    CHECK_IF_EXISTS = f"""SELECT EXISTS(SELECT * FROM {TODO_TABLE_NAME} WHERE user=? LIMIT 1)"""
    ...
    def insert_user_and_to_do_list(self, user: str, to_do_list: str):
        self.insert_query(self.INSERT_TO_DO_FOR_USER, (user, to_do_list,))

    def update_to_do_list_for_user(self, user: str, to_do_list: str):
        self.update_query(self.UPDATE_TO_DO_FOR_USER, (to_do_list, user,))
    ...
    async def update_to_do(self):
        for user, todo, in self.to_do.items():
            user = str(user)
