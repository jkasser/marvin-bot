...
last_sent) VALUES(?,?,?,?,?,?,?,?,?)"""
    ...
    UPDATE_REMINDER_LAST_SENT = (
        f"""UPDATE {TABLE_NAME} SET last_sent = ? where id = ?"""
    )
    ...
    def __init__(self, bot):
        ...
        super().__init__()
        try:
            self._create_table(self.conn, self.REMINDERS_TABLE)
            self.reminder_dict = {}
    ...
    def _insert_reminder(self, values):
        return self._insert_query(self.INSERT_REMINDER, values)

    def _mark_reminder_inactive(self, reminder_id):
        self._update_query(self.MARK_REMINDER_INACTIVE, (reminder_id,))

    def _update_last_sent_time_for_reminder_by_id(self, sub_id, last_sent):
        self._update_query(
            self.UPDATE_REMINDER_LAST_SENT,
            (
                last_sent,
                sub_id,
            ),
    ...
    def _get_all_reminders(self):
        return self._get_query(self.GET_ALL_REMINDERS)
    ...
    @commands.command(
        name="remind",
        aliases=["rem", "setreminder", "createreminder"],
    ...
    async def create_reminder(self, ctx):
        ...
        try:
            ...
            if (
                ...
                try:
                    ...
                    # if we didn't find it in our list of units, tell the user
                        ...
                        await ctx.send(
                            f"I was unable to find your provided measurement of time: {unit}. I accept"
                    ...
                    await ctx.send(
                        f"I ran into an issue processing your request. I was unable to parse {frequency} "
            ...
            await ctx.send(
                "In how long would you like to be reminded? If this is a repeat reminder, "
                "this will be the time from which I calculate all subsequent reminders. "
                "\nE.g. 2 days, 4 hours, September 25, now"
            )
            # wait for the user to reply
            start_reminder = await self.bot.wait_for(
                "message", check=check, timeout=timeout
            )
            start_reminder = start_reminder.content
            try:
                when_datetime = self._get_when_remind_date(
                    start_reminder, start_time=now
                )
            except ValueError:
                await ctx.send(
                    f"I was unable to parse your provided value of: {start_reminder.content}. Please"
                    f" try this command again. Goodbye."
                )
                return

            # ok insert it into the database and into memory
            inserted_id = self._insert_reminder(
                (
                    author_id,
                    name,
                    when_datetime,
            ...
            await ctx.send(
                "You ran out of time! Please try calling this command again! Goodbye."
            )

    @commands.command(
        name="deletereminder",
        aliases=["deleterem", "reminderdelete", "reminddelete"],
        help="Call this instead of updatereminder if you know the ID of the reminder you wish to "
        "deactivate",
    )
    ...
    @commands.command(
        name="updatereminder",
        aliases=["deactivatereminder", "updaterem", "remupdate", "remdeactivate"],
        help="Let me remind you of something! call !remind and let me guide you through setting one up!",
    )
    async def update_reminder(self, ctx):
        ...
        await ctx.send(
            "Which reminder would you like to remove? Please supply the ID of the reminder.\n"
        )
    ...
    @commands.command(
        name="getreminders",
        aliases=["remget", "getrem", "reminderget", "getreminder"],
    ...
    async def get_reminders(self, ctx):
        ...
        if msg == "":
            ...
            await channel.send(
                f"Here are your current active reminders:{new_line}{msg}"
            )
                ...
                if reminder["active"] == 1:
                    ...
                    if reminder["repeat"] == 0:
                        ...
                        if datetime.datetime.now() >= reminder["when"]:
                            ...
                            await channel.send(
                                f'{reminder["name"]}! This is your reminder to: {reminder["what"]}!'
                            )
                        ...
                        if reminder["last_sent"] == datetime.datetime.min:
                            ...
                            if datetime.datetime.now() >= reminder["when"]:
                                ...
                                self._update_last_sent_time_for_reminder_by_id(
                                    reminder["id"], last_sent
                                )
                        ...
                        elif datetime.datetime.now() >= reminder[
                            "last_sent"
                        ] + datetime.timedelta(minutes=int(reminder["frequency"])):
                            channel = self.bot.get_channel(reminder["channel"])
                            await channel.send(
                                f'{reminder["name"]}! This is your reminder to: {reminder["what"]}!'
                            )
